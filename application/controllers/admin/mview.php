...
class Mview extends CI_Controller
{
    ...
    */
    public function lists () {
		...
		$total_num = $this->base->get_data('mview')->num_rows();
		...
		$this->gpagination->target(site_url('admin/mview/lists'));

		$this->_data['pagination'] = $this->gpagination->getOutput();
		$this->_data['lists'] = $this->base->get_data('mview', array(), '*', $limit, $offset, 'sort DESC, id DESC')->result_array();
        $this->load->view('admin/mview_list', $this->_data);
    }
    ...
    public function op () {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($id = $this->input->get('id')) {
				$this->_data['row'] = $this->base->get_data('mview', array('id'=>$id))->row_array();
			}
			...
			if($id) {
				$this->base->update_data('mview', array('id' => $id), $deal_data);
			} else {
				$id = $this->base->insert_data('mview', $deal_data);
			}

			$this->msg->showmessage('添加成功', site_url('admin/mview/lists'));
		}
    }
    ...
    */
    public function del () {
        ...
        if($id && $this->base->del_data('mview', array('id' => $id))) {
        	exit('ok');
        } else {
        	exit('no');
        }
